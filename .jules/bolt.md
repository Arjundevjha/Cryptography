...
## 2025-05-24 - Unroll Round Loops and Vectorize Block Word Extraction in MD5/SHA-1
**Learning:** In block-based hash functions like MD5 and SHA-1, performing per-word `int.from_bytes` list comprehensions, helper function call dispatches (`_left_rotate32`, round function lists), and inner `if-elif` round checks inside compression loops introduces significant interpreter overhead. Utilizing `struct.unpack('<16I', ...)` / `struct.unpack('>16I', ...)` for block word extraction, inlining bitwise rotation operations directly, and unrolling compression loops into dedicated round loops with simplified bitwise equivalences yields a ~1.26x–1.39x speedup.
**Action:** Use `struct.unpack` for block word extraction, inline bitwise rotations, and unroll distinct round loops in pure Python hash compression functions.
