...
## 2025-05-24 - Pre-compute Byte Lookup Table and Memoize Block Decryption in Pure Python RSA
**Learning:** In byte-by-byte RSA encryption/decryption, computing modular exponentiations (`pow(b, e, n)` and `pow(c, d, n)`) for every byte creates severe performance bottlenecks (e.g., >5 seconds for 900 characters). Because bytes have at most 256 unique values, pre-computing a 256-entry encryption table and memoizing decrypted ciphertext blocks in a dictionary (`dec_cache`) reduces repeated $O(\log d)$ modular exponentiations to $O(1)$ lookups, yielding a ~30x speedup for RSA decryption and ~3.6x speedup for RSA encryption.
**Action:** Pre-compute byte value lookup tables and use block memoization caches when executing byte-by-byte modular arithmetic ciphers.
