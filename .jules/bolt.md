...
## 2025-05-24 - Unroll Keccak Permutation State and Vectorize SHA-3 Block Parsing
**Learning:** In Keccak/SHA-3 implementations, performing 24 rounds of Theta, Rho/Pi, Chi, and Iota step transformations using nested 2D loops over 5x5 matrices creates 72 list allocations (`[0]*5`, `[0]*25`) per block along with heavy modulo arithmetic (`(x+1)%5`, `(2*x+3*y)%5`). Unrolling the 25-lane state into scalar variables (`b0`–`b24`, `c0`–`c4`, `d0`–`d4`) and vectorizing block word extraction with `struct.unpack('<17Q', ...)` eliminates inner-loop allocations and delivers a ~2x speedup.
**Action:** Unroll fixed-dimension state matrix transformations into scalar assignments and use `struct.unpack` for block-level 64-bit word extraction.
