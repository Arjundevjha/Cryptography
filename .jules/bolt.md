...
## 2025-05-24 - Inline Round Operations & Register Unpacking in Pure-Python SHA-256 Hash Loops
**Learning:** In pure Python hash compression loops (such as SHA-256), invoking helper functions (`sigma_0_upper`, `ch_func`, `maj_func`) and indexing state arrays (`state[0]`..`state[7]`) within per-block 64-round loops introduces heavy call stack allocation and list item lookup overhead (384 function calls and 1,024 list lookups per 64-byte block). Unpacking hash state into local scalar registers (`a, b, c, d, e, f, g, h_val`) and inlining bitwise rotation and logic functions yields a ~1.45x speedup.
**Action:** Unpack list-based state arrays into scalar local variables and inline bitwise logic in high-iteration cryptographic compression loops.
