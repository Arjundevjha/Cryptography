...
## 2025-05-24 - Pre-compute Digraph Transformation Tables for Playfair Cipher
**Learning:** In digraph-based grid ciphers like Playfair, evaluating per-pair 2D matrix positions (`pos_map`), conditional row/column branches, modulo arithmetic, and 2D indexing inside character loops creates substantial Python bytecode overhead. Because the alphabet contains only 25 letters, there are only $25 \times 25 = 625$ possible 2-character pairs. Precomputing a 625-entry digraph pair transformation dictionary (`transform_map`) for a given key reduces per-pair processing to $O(1)$ lookups, yielding a ~1.8x–2.0x overall encryption/decryption speedup.
**Action:** Pre-compute small fixed-size pair transformation lookup maps for digraph/block ciphers to avoid conditional branching and matrix arithmetic in message loops.
