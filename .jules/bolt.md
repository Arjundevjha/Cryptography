...
## 2025-05-24 - Pre-filter Composite Candidates with Small Primes & Bitwise Factorization
**Learning:** In primality testing for RSA key generation, invoking Miller-Rabin modular exponentiations (`pow(a, d, val)`) on every candidate odd integer dominates runtime (~177 tests per 512-bit prime). Doing fast C-level integer modulo checks (`val % p == 0`) against pre-computed small primes (up to 349) filters out ~88% of composite candidates early. Combining this with bitwise factor decomposition (`r = (v_minus_1 & -v_minus_1).bit_length() - 1` and `d = v_minus_1 >> r`) eliminates loop interpretation overhead and achieves a ~5.3x speedup for RSA keypair generation.
**Action:** Always pre-filter prime candidates with small primes trial division and use bitwise arithmetic for power-of-two factor decomposition in primality testing.
