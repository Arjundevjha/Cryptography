...
## 2025-05-24 - Inline Bitwise Rotations and Unroll Cyclic Variable Shifts in Block Hashing
**Learning:** In pure Python block compression hash functions like SHA-256, calling helper functions (`_rotr32`, `ch_func`, `maj_func`, `sigma`) inside 64-round compression loops creates hundreds of call stack allocations per 64-byte block (e.g. 384+ function calls per block). Inlining 32-bit bitwise rotation math, using `struct.unpack('>16I', block_bytes)` for single-pass word unpacking, simplifying boolean logic (`ch = g ^ (e & (f ^ g))`), and unrolling cyclic variable shifts (`a..h`) across 8 rounds per iteration eliminates loop register swapping and delivers a ~1.6x speedup.
**Action:** Use `struct.unpack` for block unpacking, inline bitwise rotation math, and unroll cyclic register shifts in compression loops for hash functions.
