...
## 2025-05-24 - Inline Rotations and Unroll Variable Shifts in SHA-256 Block Compression
**Learning:** In pure Python SHA-256 implementations, calling helper functions (`_rotr32`, `_ch_func`, `_maj_func`), unpacking words with `int.from_bytes` inside a per-word loop, and recreating state lists (`state = list(h_state)`) on every block compression creates heavy bytecode call stack and memory allocation overhead. Unpacking 16 words with `struct.unpack('>16I', block_bytes)`, inlining bitwise right-rotations, simplifying `ch`/`maj` boolean expressions, and unrolling state variable shifts across 8 rounds delivers a ~1.68x speedup.
**Action:** Unpack block words using `struct.unpack`, inline bitwise rotations, simplify boolean logic, and unroll cyclic variable shifts in hash compression loops.
