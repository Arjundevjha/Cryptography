...
## 2025-05-24 - Eliminate String Slicing in Enigma Rotor Notch Checks
**Learning:** Accessing property getters like `Rotor.left` (`"ABC..."[off:] + "ABC..."[:off]`) inside tight character enciphering loops allocates temporary 26-character strings up to 4 times per character during double-stepping notch checks (`self.r2.left[0] == self.r2.notch`). Storing integer `notch_code = ord(notch) - 65` on `Rotor` and using direct modular offset integer comparisons `(r2.offset % 26) == r2.notch_code` along with local variable bindings yields a ~1.77x speedup (from 300,000 to 531,000 chars/sec).
**Action:** Store integer notch/position codes and use modular arithmetic directly instead of string-slicing state properties during stateful wheel notch checks.
