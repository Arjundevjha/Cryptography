...
## 2025-05-24 - Inline AES Block State Transformations and Unroll State Variables
**Learning:** In pure Python block ciphers like AES, calling per-round helper functions (`sub_bytes`, `shift_rows`, `mix_columns`, `add_round_key`) and constructing intermediate 16-element lists on every round creates substantial Python call stack and object allocation overhead (over 50 function calls and list allocations per block). Unrolling the 16-byte state into scalar local variables (`s0`..`s15`) and directly inlining state transformations yields a ~2.3x performance speedup.
**Action:** Unroll fixed block states into scalar local variables and inline per-round matrix/box transformations to eliminate Python function call overhead and temporary list allocations in block ciphers.
