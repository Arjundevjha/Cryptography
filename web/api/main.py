...
async def aes_decrypt_endpoint(data: AesDecryptInput):
    ...
    try:
        ...
        logger.error("Invalid hex format for ciphertext or nonce in AES decrypt", exc_info=ve)
    ...
    try:
        ...
        logger.error("AES decryption failed: %s", e, exc_info=True)
        raise HTTPException(status_code=400, detail=str(e))
...
def rsa_encrypt(data: RsaEncryptInput):
    try:
        ...
        logger.error("RSA encryption failed: %s", e, exc_info=True)
        raise HTTPException(status_code=400, detail=str(e))
...
def rsa_decrypt(data: RsaDecryptInput):
    try:
        ...
        logger.error("RSA decryption failed: %s", e, exc_info=True)
        raise HTTPException(status_code=400, detail=str(e))
