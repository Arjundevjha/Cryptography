...
async def aes_decrypt_endpoint(data: AesDecryptInput):
    ...
    try:
        ...
        logger.error("Error decoding hex strings in AES decrypt: %s", ve, exc_info=True)
    ...
    try:
        ...
        logger.error("Error decrypting AES ciphertext: %s", e, exc_info=True)
        raise HTTPException(status_code=400, detail=str(e))
...
def rsa_encrypt(data: RsaEncryptInput):
    try:
        ...
        logger.error("Error encrypting RSA plaintext: %s", e, exc_info=True)
        raise HTTPException(status_code=400, detail=str(e))
...
def rsa_decrypt(data: RsaDecryptInput):
    try:
        ...
        logger.error("Error decrypting RSA ciphertext: %s", e, exc_info=True)
        raise HTTPException(status_code=400, detail=str(e))
