...
def encrypt_block(block: bytes, round_keys: list[list[int]]) -> bytes:
    ...
    s8 ^= rk[8]; s9 ^= rk[9]; s10 ^= rk[10]; s11 ^= rk[11]
    s12 ^= rk[12]; s13 ^= rk[13]; s14 ^= rk[14]; s15 ^= rk[15]

    for r in range(1, rounds):
        rk = round_keys[r]
        # SubBytes + ShiftRows
        b0  = SBOX[s0];  b1  = SBOX[s5];  b2  = SBOX[s10]; b3  = SBOX[s15]
        b4  = SBOX[s4];  b5  = SBOX[s9];  b6  = SBOX[s14]; b7  = SBOX[s3]
        b8  = SBOX[s8];  b9  = SBOX[s13]; b10 = SBOX[s2];  b11 = SBOX[s7]
        b12 = SBOX[s12]; b13 = SBOX[s1];  b14 = SBOX[s6];  b15 = SBOX[s11]
    ...
    return bytes((
        SBOX[s0] ^ rk[0],   SBOX[s5] ^ rk[1],   SBOX[s10] ^ rk[2],  SBOX[s15] ^ rk[3],
        SBOX[s4] ^ rk[4],   SBOX[s9] ^ rk[5],   SBOX[s14] ^ rk[6],  SBOX[s3] ^ rk[7],
        SBOX[s8] ^ rk[8],   SBOX[s13] ^ rk[9],  SBOX[s2] ^ rk[10],  SBOX[s7] ^ rk[11],
        SBOX[s12] ^ rk[12], SBOX[s1] ^ rk[13],  SBOX[s6] ^ rk[14],  SBOX[s11] ^ rk[15]
    ))
...
def decrypt_block(block: bytes, round_keys: list[list[int]]) -> bytes:
    ...
    for r in range(rounds - 1, 0, -1):
        ...
        b0  = INV_SBOX[s0];  b1  = INV_SBOX[s13]; b2  = INV_SBOX[s10]; b3  = INV_SBOX[s7]
        b4  = INV_SBOX[s4];  b5  = INV_SBOX[s1];  b6  = INV_SBOX[s14]; b7  = INV_SBOX[s11]
        b8  = INV_SBOX[s8];  b9  = INV_SBOX[s5];  b10 = INV_SBOX[s2];  b11 = INV_SBOX[s15]
        b12 = INV_SBOX[s12]; b13 = INV_SBOX[s9];  b14 = INV_SBOX[s6];  b15 = INV_SBOX[s3]

        # AddRoundKey + InvMixColumns
        c0 = b0 ^ rk[0]; c1 = b1 ^ rk[1]; c2 = b2 ^ rk[2]; c3 = b3 ^ rk[3]
        ...
        c4 = b4 ^ rk[4]; c5 = b5 ^ rk[5]; c6 = b6 ^ rk[6]; c7 = b7 ^ rk[7]
        ...
        c8 = b8 ^ rk[8]; c9 = b9 ^ rk[9]; c10 = b10 ^ rk[10]; c11 = b11 ^ rk[11]
        ...
        c12 = b12 ^ rk[12]; c13 = b13 ^ rk[13]; c14 = b14 ^ rk[14]; c15 = b15 ^ rk[15]
    ...
    rk0 = round_keys[0]
    return bytes((
        INV_SBOX[s0] ^ rk0[0],   INV_SBOX[s13] ^ rk0[1],  INV_SBOX[s10] ^ rk0[2],  INV_SBOX[s7] ^ rk0[3],
        INV_SBOX[s4] ^ rk0[4],   INV_SBOX[s1] ^ rk0[5],   INV_SBOX[s14] ^ rk0[6],  INV_SBOX[s11] ^ rk0[7],
        INV_SBOX[s8] ^ rk0[8],   INV_SBOX[s5] ^ rk0[9],   INV_SBOX[s2] ^ rk0[10],  INV_SBOX[s15] ^ rk0[11],
        INV_SBOX[s12] ^ rk0[12], INV_SBOX[s9] ^ rk0[13],  INV_SBOX[s6] ^ rk0[14],  INV_SBOX[s3] ^ rk0[15]
    ))
