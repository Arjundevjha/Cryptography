...
# (`B64_DECODE_LUT` and `PAIR_LUT`). Combined with pre-allocated bytearrays, this
# delivers ~1.86x performance speedup.
def _init_b64_luts():
    """Pre-compute lookup tables for Base64 decoding."""
    b64_dec = [-1] * 256
    for idx, char in enumerate(BASE64_CHARS):
        b64_dec[ord(char)] = idx

    pair_dec = [-1] * 65536
    for idx0, char0 in enumerate(BASE64_CHARS):
        for idx1, char1 in enumerate(BASE64_CHARS):
            pair_dec[(ord(char0) << 8) | ord(char1)] = (idx0 << 6) | idx1
    return b64_dec, pair_dec


B64_DECODE_LUT, PAIR_LUT = _init_b64_luts()
...
def b64decode(data_str: str) -> bytes:
    ...
    string searches (`.index()`), sub-slicing, and dynamic array reallocation overhead
    (~1.86x speedup).
    """
