...
from methods.modern.symmetric import (
    ...
    sub_word, rot_word, xtime, mul_gf,
...
def test_add_round_key():
    """Test XOR state with round key in AES with known values and properties."""
    ...
    # Self-XOR property: state XOR state == all zeros
    assert add_round_key(state, state) == zero_key

    # Bit complement property: state XOR all 0xFF == inverted state
    assert add_round_key(state, [0xff] * 16) == [b ^ 0xff for b in state]
